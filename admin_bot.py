...
async def got_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if url:
        ...
        msg = "Photo " + str(count) + " uploaded!\n\nSend more photos (up to 6)\nor press Continue"
...
async def more_photo_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(images) >= 6:
        await update.message.reply_text("Max 6 photos. Press Continue")
        return MORE_PHOTOS
    await update.message.reply_text("⏳ Загружаю фото...")
    url = await upload_photo(context.bot, update.message.photo[-1].file_id)
    if url:
        ...
        count = len(images)
        kb = ReplyKeyboardMarkup([["Continue"]], resize_keyboard=True, one_time_keyboard=True)
        msg2 = "Photo " + str(count) + " uploaded!\n\nSend more or press Continue"
        await update.message.reply_text(msg2, reply_markup=kb)
    else:
